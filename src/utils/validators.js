...
function validateItem(item) {
  ...
  if (!Number.isInteger(item.quantity) || item.quantity <= 0) throw make('Item.quantity must be a positive integer');
}
function validateCheckoutPayload(payload) {
  if (!payload || typeof payload !== 'object') throw make('Payload must be an object');
  if (!Array.isArray(payload.items) || payload.items.length === 0) throw make('items must be non-empty');
...
function make(msg) { const e = new Error(msg); e.status = 400; return e; }
